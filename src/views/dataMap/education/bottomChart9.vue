<template>
  <div class="echartsContainer" id="bottomChart9Container">
    <span class="title_box" :class="{'visibilityNone':false}">本季度教学指标</span>
    <div id="bottomChart9" :class="{'visibilityNone':isError}" class="educationEcharts"></div>
...
export default {
  ...
  mounted() {
    ...
    this.$nextTick(function() {
      self.init("bottomChart9", self.bottomChart9Option);
    });
  },
  methods: {
    init(id, opt) {
    ...
    // 获取数据
    getData(cityName) {
      var myChart = this.$echarts.init(document.getElementById("bottomChart9"));
      ...
      this.$store
        ...
        .then(xxdata => {
          this.bottomChart9Option.legend.data = [];
          this.bottomChart9Option.series[0].data = [];
          let self = this;
          // xxdata.bottomChart9["result"].map(function(val) {
          if (
            xxdata.data != null &&
            ...
            xxdata.data["result"].map(function(val) {
              self.bottomChart9Option.legend.data.push(val["system"]);
              self.bottomChart9Option.series[0].data.push({
                value: val["systemNum"],
                name: val["system"],
          ...
          // this.init("bottomChart9", this.bottomChart9Option);
          myChart.setOption(this.bottomChart9Option);
          myChart.hideLoading();
        })
        .catch(err => {
          this.isError = true;
