<template>
  <div class="echartsContainer" id="termTypeRadioContainer">
    <span class="title_box" :class="{'visibilityNone':false}">{{$t('echartsTitle.DeviceTypeRatio')}}</span>
    <div id="termTypeRadio" :class="{'visibilityNone':isError}"></div>
  ...
  mounted() {
    ...
    this.$nextTick(function() {
      ...
      this.termTypeRadioOption.toolbox.feature.myTool3.onclick = function() {
        var echartsId = "termTypeRadio",
          echartsHtml =
            '<span class="title_box">设备类型比例</span><div id="termTypeRadio"></div>',
      ...
      this.termTypeRadioOption.toolbox.feature.myTool4.onclick = function() {
        ...
        if (cityCode === undefined) {
          window.open("#/dataMap/oneEcharts/termTypeRadio");
        } else {
          window.open("#/dataMap/oneEcharts/termTypeRadio/" + cityCode);
        }
      };
  ...
  methods: {
    ...
    getData(cityName) {
      var myChart = this.$echarts.init(
        document.getElementById("termTypeRadio")
      );
      ...
      this.$store
        ...
        .then(xxdata => {
          ...
          let self = this;
          // xxdata.termTypeRadio["result"].map(function(val) {
          if (
            ...
            xxdata.data["result"].map(function(val) {
              self.termTypeRadioOption.legend.data.push(val["system"]);
          ...
          // this.init("termTypeRadio", this.termTypeRadioOption);
